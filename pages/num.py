...
class NumPage(Screen):
    ...
    def process_data_numbd(self, input_text):
        ...
        output_lines = [header]  # Начинаем с заголовка
        for i, line in enumerate(lines, start=1):  # Нумерация строк начинается с 1
            try:
                ...
                if "Balance refill" in line or "Phone number verification" in line:
                    output_lines.append(line)  # Добавляем строку без изменений
                    continue
                
                # Разделяем строку на части
                parts = line.split(",")
                if len(parts) < 5:  # Проверка на количество частей
                    raise ValueError(f"Ошибка в строке {i}: недостаточно данных.")
    ...
    def process_data_numadmn(self, input_text):
        ...
        for i, line in enumerate(lines, start=1):  # Нумерация строк начинается с 1
            try:
                ...
                # Разделяем строку на части
                parts = line.split("\t")
                if len(parts) < 3:  # Проверка на количество частей
                    raise ValueError(f"Ошибка в строке {i}: недостаточно данных.")
